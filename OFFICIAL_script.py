...
CONFIG_FILE_NAME = '/home/martin/Documents/Syntheses/src/config.yml'
...
def main():
    ...
    with open(CONFIG_FILE_FULL_PATH, 'r') as stream:
        try:
            ...
            # document['output_base'] = '/home/martin/Documents/pdf'
            document['output_base'] = '/home/martin/Documents/OneDriveEPL/EPL-Drive'
            ...
            for basename, dirname, translated_properties in filtered_array:
                ...
                try:
                    ...
                    subprocess.call(translated_properties['buildCommand2'], shell=True, cwd=dirname, stdout=DEVNULL)
            ...
            try:
                remove_temp_files(CONFIG_FILE_LOCATION)
            except subprocess.CalledProcessError:
...
def remove_temp_files(output_folder):
    remove_tuple = ('.bcf', 'out', '.gnuplot', '.tdo', '.xml', '.aux', '.log', '.synctex.gz', '.toc', '.auxlock')
...
def build_command(file, out_folder, mapping_dictionary):
    ...
    if result:
        ...
        if file.endswith(("All.tex", "summary.tex", "Maj.tex", "Min.tex", "notes.tex", "exercices.tex")):
                ...
                if check_dictionary(translated_properties, 'courseLabel') and check_dictionary(translated_properties,
                    ...
                    if check_dictionary(translated_properties, 'type'):
                        ...
                        if check_dictionary(translated_properties, 'year') \
                            ...
                            elif translated_properties['type'] == "APE":
                                if dirname[-1] == 's':
                                    out_folder_name = 'APE' + '-' + basename.split('-')[0] +'-' + translated_properties['courseLabel'] + '-Sol'
                                elif dirname[-3:] == 'mcq':
                                    out_folder_name = 'QCM' + '-' + basename.split('-')[0] +'-' + translated_properties['courseLabel'] + '-Sol'
                                else:
                                    out_folder_name = 'APE' + '-' + basename.split('-')[0] +'-' + translated_properties['courseLabel'] + '-Sol' + dirname[-1]
                            else:
...
#        out_folder_name = 'CM-tdo-ECGE1317' 
    ...
    sub_build_command = 'pdflatex -interaction nonstopmode -output-format {} -jobname {} {}'
    translated_properties['buildCommand'] = sub_build_command.format(
            'pdf',
            pipes.quote(out_folder_name),
            pipes.quote(basename)
            )
    sub_build_command = 'cp {} {}'
    translated_properties['buildCommand2'] = sub_build_command.format(
            pipes.quote(out_folder_name+'.pdf'),
            pipes.quote(translated_properties['folderPath'])
            )
    
    # Exam/test without solution
    if translated_properties['type'] in ('Examens','Interros'):
        sub_build_command = 'rm -rf {} && latexmk -pdf -pdflatex="pdflatex -shell-escape -enable-write18 {}" -use-make {} && cp {} {}/{}'
        #translated_properties.update('buildCommandnotsol')
        translated_properties['buildCommandnotsol'] = sub_build_command.format(
            pipes.quote(basename[:-3]+'pdf'),
            pipes.quote('\def\Sol{false} \input{%S}'),
            pipes.quote(basename),
            pipes.quote(basename[:-3]+'pdf'),
            pipes.quote(translated_properties['folderPath']),
            pipes.quote(out_folder_name[:-4]+'.pdf')
            )
