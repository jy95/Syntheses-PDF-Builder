...
def main():
    ...
    with open(CONFIG_FILE_NAME, 'r') as stream:
        ...
        if typ == 'exam' or typ == 'test':
            ...
            out_path = cycle + '/' + option_map + '/Q' + str(quadri) + '/L' + option + str(code) + ' - ' + titre + '/' + type_map + '/' + str(year) + '_' + month.title()
        else:
            type_map = document['clients'][0]['output']['parameters'][1]['mapping'][typ]
            out_path = cycle + '/' + option_map + '/Q' + str(quadri) + '/L' + option + str(code) + ' - ' + titre + '/' + type_map
